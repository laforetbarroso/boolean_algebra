...
(* Why3 goal *)
Theorem all_set_without_bot_eval_set'vc :
  forall (c:set.Fset.fset atomicformula), ~ set.Fset.mem ABot c ->
  (~ set.Fset.is_empty c ->
   (let result := set.Fset.remove (set.Fset.pick c) c in
    ((0%Z <= (set.Fset.cardinal c))%Z /\
     ((set.Fset.cardinal result) < (set.Fset.cardinal c))%Z) /\
    ~ set.Fset.mem ABot result)) /\
  (set.Fset.is_empty c \/
   ((eval_setconjunction (set.Fset.remove (set.Fset.pick c) c) val_all_top) =
    top) ->
