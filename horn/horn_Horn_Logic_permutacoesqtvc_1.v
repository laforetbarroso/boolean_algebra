...
Parameter clauselist2:
  Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type.

Parameter c: set.Fset.fset atomicformula.

Axiom Requires : list.Permut.permut clauselist clauselist2.

Axiom Requires1 :
  list.Permut.permut (Init.Datatypes.app clauselist processed_clauses)
  full_clauselist.

Axiom Requires2 : set.Fset.mem ATop c.

Axiom Requires3 : ~ set.Fset.mem ABot c.

Axiom Requires4 : sublist clauselist full_clauselist.
...
Parameter result: set.Fset.fset atomicformula.
...
Axiom H3 :
  ...
  match x1 with
  | RProp t =>
      ((t = top) -> (result1 = ATop)) /\ (~ (t = top) -> (result1 = ABot))
...
Axiom H5 :
  let o := eq_pos1 (convertRStoAF x1) ABot in
  ((o = Init.Datatypes.true) -> (result = (set.Fset.add ABot c))) /\
  (~ (o = Init.Datatypes.true) ->
   (let result1 := convertRStoAF x1 in
    ...
    (let o1 := set.Fset.add result1 c in
     let o2 := removeFromList (x, x1) clauselist in
     let o3 := Init.Datatypes.cons (x, x1) processed_clauses in
     set.Fset.infix_eqeq (algorithmA_spec full_clauselist o3 o2 o1)
     (algorithmA_spec full_clauselist o3 (removeFromList (x, x1) clauselist2)
      o1) /\
     set.Fset.infix_eqeq result (algorithmA_spec full_clauselist o3 o2 o1)))).

Axiom h : ((findOneSubset clauselist c) = (findOneSubset clauselist2 c)).
