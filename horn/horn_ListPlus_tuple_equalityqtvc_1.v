...
(* Why3 goal *)
Theorem tuple_equality'vc {a:Type} {a_WT:WhyType a} :
  ...
  | Init.Datatypes.cons x x1 =>
      match z1 with
      | Init.Datatypes.cons x2 x3 =>
          (0%Z <= (list.Length.length r1))%Z /\
          ((list.Length.length x1) < (list.Length.length r1))%Z
      | _ => True
      end
  end /\
  (match r1 with
   ...
   | Init.Datatypes.cons x x1 =>
       match z1 with
       | Init.Datatypes.cons x2 x3 =>
           (is_equal_list (Init.Datatypes.app x3 z2)
            (Init.Datatypes.app x1 r2) -> is_equal_list x3 x1 ->
            is_equal_list z2 r2) /\
           (is_equal_list (Init.Datatypes.app x3 z2)
            (Init.Datatypes.app x1 r2) -> is_equal_list z2 r2 ->
            is_equal_list x3 x1)
       | _ => True
       end
   end ->
