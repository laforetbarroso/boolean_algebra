...
Axiom H : (clauselist = (Init.Datatypes.cons x Init.Datatypes.nil)).
...
Parameter s: set.Fset.fset atomicformula.

Parameter p: rightside.

Axiom Hinst1 : ((findOneSubset clauselist c) = (Init.Datatypes.Some (s, p))).
...
Axiom Hinst2 :
  ...
  (~ ((eq_pos1 (convertRStoAF p) ABot) = Init.Datatypes.true) ->
   ((algorithmA_spec full_clauselist processed_clauses clauselist c) =
    (algorithmA_spec full_clauselist
     (Init.Datatypes.cons (s, p) processed_clauses)
     (removeFromList (s, p) clauselist) (set.Fset.add (convertRStoAF p) c)))).
