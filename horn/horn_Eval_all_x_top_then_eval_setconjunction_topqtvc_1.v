...
(* Why3 goal *)
Theorem all_x_top_then_eval_setconjunction_top'vc :
  forall (s:set.Fset.fset atomicformula) (f:i -> b),
  (~ set.Fset.is_empty s ->
   (0%Z <= (set.Fset.cardinal s))%Z /\
   ((set.Fset.cardinal (set.Fset.remove (set.Fset.pick s) s)) <
    (set.Fset.cardinal s))%Z) /\
  (set.Fset.is_empty s \/
   (let result := set.Fset.remove (set.Fset.pick s) s in
    (forall (x:atomicformula), set.Fset.mem x result ->
