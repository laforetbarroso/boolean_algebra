...
Axiom Requires : is_unsatisfiable_clauselist full_clauselist.

Axiom Requires1 :
  list.Permut.permut (Init.Datatypes.app processed_clauses clauselist)
  full_clauselist.

Axiom Requires2 : sublist clauselist full_clauselist.

Axiom Requires3 :
  set.Fset.subset c (set.Fset.add ATop (set_smb_clauselist full_clauselist)).

Axiom Requires4 : is_satisfiable_clauselist processed_clauses.
...
Axiom Requires7 : set.Fset.mem ATop c.

Axiom Requires8 : ~ set.Fset.mem ABot c.
...
Axiom H : (clauselist = (Init.Datatypes.cons x (Init.Datatypes.cons w w1))).

Parameter x1: set.Fset.fset atomicformula.

Parameter x2: rightside.

Axiom H1 : ((findOneSubset clauselist c) = (Init.Datatypes.Some (x1, x2))).

Axiom H2 :
  forall (f:i -> b),
  ((eval_rightside (assign_rightside x2 f)) =
   (eval_atomicformula (convertRStoAF x2) f)).

Axiom H3 :
  let result := convertRStoAF x2 in
  match x2 with
  | RProp x3 =>
      ((x3 = top) -> (result = ATop)) /\ (~ (x3 = top) -> (result = ABot))
  | RVar x3 => (result = (AVar x3))
  end.

Axiom H4 :
  let result := ABot in
  let result1 := convertRStoAF x2 in
...
Axiom H5 :
  ...
  (let result := convertRStoAF x2 in
   ...
   (algorithmA_spec full_clauselist
    ...
    (removeFromList (x1, x2) clauselist) (set.Fset.add result c))).

(* Why3 goal *)
Theorem unsat_cl_mem_ABot_in_algorithmA'vc :
  set.Fset.mem ABot
  (algorithmA_spec full_clauselist processed_clauses clauselist c).
