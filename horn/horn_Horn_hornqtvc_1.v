...
Parameter algorithmA:
  Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type ->
  Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type ->
  Init.Datatypes.list (set* rightside)%type -> set -> set.

Axiom algorithmA'spec :
  forall (full_clauselist:
          Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type)
    (processed_clauses:
     Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type)
    (clauselist:Init.Datatypes.list (set* rightside)%type) (c:set),
  set.Fset.mem ATop (to_fset c) -> ~ set.Fset.mem ABot (to_fset c) ->
  ...
  (forall (f:i -> b),
   ((eval_clauselist processed_clauses f) = top) <->
   ((eval_setconjunction (to_fset c) f) = top)) ->
  ((to_fset (algorithmA full_clauselist processed_clauses clauselist c)) =
   (algorithmA_spec full_clauselist processed_clauses
    (listset_to_listfset clauselist) (to_fset c))).

(* Why3 goal *)
Theorem horn'vc :
  forall (clauselist:Init.Datatypes.list (set* rightside)%type),
  forall (o:set),
  ((to_fset o) =
   (set.Fset.add ATop (set.Fset.empty : set.Fset.fset atomicformula))) /\
  ((set.Fset.cardinal (to_fset o)) = 1%Z) ->
  let result := Init.Datatypes.nil in
  let result1 := listset_to_listfset clauselist in
  (set.Fset.mem ATop (to_fset o) /\
   ~ set.Fset.mem ABot (to_fset o) /\
   sublist (listset_to_listfset clauselist) result1 /\
   list.Permut.permut
   (Init.Datatypes.app (listset_to_listfset clauselist) result) result1 /\
   set.Fset.subset (to_fset o)
   (set.Fset.add ATop (set_smb_clauselist result1)) /\
   is_satisfiable_clauselist result /\
   (forall (f:i -> b), ((eval_clauselist result1 f) = top) ->
    ((eval_setconjunction (to_fset o) f) = top)) /\
   (forall (f:i -> b),
    ((eval_clauselist result f) = top) <->
    ((eval_setconjunction (to_fset o) f) = top))) /\
  (let o1 := algorithmA result1 result clauselist o in
   ((to_fset o1) =
    (algorithmA_spec result1 result (listset_to_listfset clauselist)
     (to_fset o))) ->
   (set.Fset.mem ABot (to_fset o1) ->
    (Init.Datatypes.false = (horn_logic (listset_to_listfset clauselist)))) /\
   (~ set.Fset.mem ABot (to_fset o1) ->
    (Init.Datatypes.true = (horn_logic (listset_to_listfset clauselist))))).
