...
(* Why3 goal *)
Theorem horn_logic'vc :
  forall (clauselist:
          Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type),
  let o :=
  set.Fset.add ATop (set.Fset.empty : set.Fset.fset atomicformula) in
  let result := Init.Datatypes.nil in
  set.Fset.mem ATop o /\
  ~ set.Fset.mem ABot o /\
  sublist clauselist clauselist /\
  list.Permut.permut (Init.Datatypes.app result clauselist) clauselist /\
  set.Fset.subset o (set.Fset.add ATop (set_smb_clauselist clauselist)) /\
  (forall (f:i -> b), ((eval_clauselist clauselist f) = top) ->
   ((eval_setconjunction o f) = top)) /\
  is_satisfiable_clauselist result /\
  (forall (f:i -> b),
   ((eval_clauselist result f) = top) <-> ((eval_setconjunction o f) = top)).
