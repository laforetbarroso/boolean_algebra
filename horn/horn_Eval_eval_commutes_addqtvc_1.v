...
(* Why3 goal *)
Theorem eval_commutes_add'vc :
  forall (s:set.Fset.fset atomicformula) (x:atomicformula) (f:i -> b),
  (~ set.Fset.is_empty s ->
   (let y := set.Fset.pick (set.Fset.add x s) in
    let o := eq_pos1 x y in
    (o = Init.Datatypes.true) <-> (x = y) -> ~ (o = Init.Datatypes.true) ->
    (0%Z <= (set.Fset.cardinal (set.Fset.add x s)))%Z /\
    ((set.Fset.cardinal
      (set.Fset.add x (set.Fset.remove y (set.Fset.add x s))))
     < (set.Fset.cardinal (set.Fset.add x s)))%Z)) /\
  (set.Fset.is_empty s \/
   (let y := set.Fset.pick (set.Fset.add x s) in
    let o := eq_pos1 x y in
    ((o = Init.Datatypes.true) <-> (x = y)) /\
    ((o = Init.Datatypes.true) \/
     (let new_s := set.Fset.remove y (set.Fset.add x s) in
      ((eval_setconjunction (set.Fset.add x new_s) f) =
       (infix_slasbs (eval_atomicformula x f) (eval_setconjunction new_s f)))))) ->
