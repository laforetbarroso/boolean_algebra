...
Parameter if_term:
  set -> rightside -> set -> rightside -> Init.Datatypes.bool.

Parameter if_term1:
  set -> rightside -> set -> rightside -> Init.Datatypes.bool.

Axiom if_term'def :
  forall (set1:set) (p:rightside) (s1:set) (p1:rightside),
  (set.Fset.infix_eqeq (to_fset s1) (to_fset set1) ->
   ((if_term set1 p s1 p1) = (rs_eq_pos p1 p))) /\
  (~ set.Fset.infix_eqeq (to_fset s1) (to_fset set1) ->
   ((if_term set1 p s1 p1) = Init.Datatypes.false)).

Axiom if_term'def1 :
  forall (set1:set) (p:rightside) (s1:set) (p1:rightside),
  (set.Fset.infix_eqeq (to_fset s1) (to_fset set1) ->
   ((if_term1 set1 p s1 p1) = (rs_eq_pos p1 p))) /\
  (~ set.Fset.infix_eqeq (to_fset s1) (to_fset set1) ->
   ((if_term1 set1 p s1 p1) = Init.Datatypes.false)).

(* Why3 goal *)
Theorem removeFromList_prog'vc :
  forall (set1:set) (p:rightside)
    (clauselist:Init.Datatypes.list (set* rightside)%type),
  match clauselist with
  | Init.Datatypes.nil => True
  | Init.Datatypes.cons (s1, p1) l =>
      let o := if_term set1 p s1 p1 in
      ~ set.Fset.infix_eqeq (to_fset s1) (to_fset set1) \/
      ((o = Init.Datatypes.true) <-> (p1 = p)) ->
      ~ (o = Init.Datatypes.true) ->
      (0%Z <= (list.Length.length clauselist))%Z /\
      ((list.Length.length l) < (list.Length.length clauselist))%Z
  end /\
  (forall (result:Init.Datatypes.list (set* rightside)%type),
   match clauselist with
   | Init.Datatypes.nil => (result = Init.Datatypes.nil)
   | Init.Datatypes.cons (s1, p1) l =>
       let o := if_term1 set1 p s1 p1 in
       (~ set.Fset.infix_eqeq (to_fset s1) (to_fset set1) \/
        ((o = Init.Datatypes.true) <-> (p1 = p))) /\
       ((o = Init.Datatypes.true) /\ (result = l) \/
        ~ (o = Init.Datatypes.true) /\
        (exists result1:Init.Datatypes.list (set* rightside)%type,
         ((listset_to_listfset result1) =
          (removeFromList (to_fset set1, p) (listset_to_listfset l))) /\
         (result = (Init.Datatypes.cons (s1, p1) result1))))
   end ->
