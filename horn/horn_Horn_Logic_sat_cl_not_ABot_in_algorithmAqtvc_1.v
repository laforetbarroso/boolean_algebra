...
(* Why3 goal *)
Theorem sat_cl_not_ABot_in_algorithmA'vc :
  forall (full_clauselist:
          Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type)
    (processed_clauses:
     Init.Datatypes.list (set.Fset.fset atomicformula* rightside)%type)
  ...
  match clauselist with
  | Init.Datatypes.nil => True
  | Init.Datatypes.cons _ Init.Datatypes.nil => True
  | Init.Datatypes.cons _ _ =>
      match findOneSubset clauselist c with
      | Init.Datatypes.None => True
      | Init.Datatypes.Some (s, p) =>
          ...
          (let o := eq_pos1 result1 result in
           (o = Init.Datatypes.true) <-> (result1 = result) ->
           ...
           (let result2 := convertRStoAF p in
            ...
            (let o1 := set.Fset.add result2 c in
             let o2 := removeFromList (s, p) clauselist in
             let o3 := Init.Datatypes.cons (s, p) processed_clauses in
             ((0%Z <= (list.Length.length clauselist))%Z /\
              ((list.Length.length o2) < (list.Length.length clauselist))%Z) /\
             is_satisfiable_clauselist full_clauselist /\
             list.Permut.permut (Init.Datatypes.app o3 o2) full_clauselist /\
             sublist o2 full_clauselist /\
             set.Fset.subset o1
             (set.Fset.add ATop (set_smb_clauselist full_clauselist)) /\
             (forall (f:i -> b),
              ((eval_clauselist full_clauselist f) = top) ->
              ((eval_setconjunction o1 f) = top)) /\
             (forall (f:i -> b),
              ((eval_setconjunction o1 f) = top) <->
              ((eval_clauselist o3 f) = top)) /\
             set.Fset.mem ATop o1 /\ ~ set.Fset.mem ABot o1)))
      end
  end /\
  (match clauselist with
   | Init.Datatypes.nil => True
            ...
            ((o = Init.Datatypes.true) \/
             (let result2 := convertRStoAF p in
              ...
              ~ set.Fset.mem ABot
                (algorithmA_spec full_clauselist
                 (Init.Datatypes.cons (s, p) processed_clauses)
                 (removeFromList (s, p) clauselist) (set.Fset.add result2 c)))))
       end
   end ->
   ~ set.Fset.mem ABot
     (algorithmA_spec full_clauselist processed_clauses clauselist c)).
