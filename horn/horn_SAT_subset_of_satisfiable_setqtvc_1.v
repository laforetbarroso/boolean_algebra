...
(* Why3 goal *)
Theorem subset_of_satisfiable_set'vc :
  forall (c:set.Fset.fset atomicformula) (d:set.Fset.fset atomicformula)
    (f:i -> b),
  ((eval_setconjunction d f) = top) /\ set.Fset.subset c d ->
  (~ set.Fset.is_empty d -> ~ set.Fset.is_empty c ->
   (let o := set.Fset.pick d in
    let result := set.Fset.remove o d in
    ((0%Z <= (set.Fset.cardinal d))%Z /\
     ((set.Fset.cardinal result) < (set.Fset.cardinal d))%Z) /\
    ((eval_setconjunction result f) = top) /\
    set.Fset.subset (set.Fset.remove o c) result)) /\
  (set.Fset.is_empty d \/
   set.Fset.is_empty c \/
   ((eval_setconjunction (set.Fset.remove (set.Fset.pick d) c) f) = top) ->
