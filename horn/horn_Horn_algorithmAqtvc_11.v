...
Axiom Ensures :
  match Init.Datatypes.Some (x, x1) with
  | Init.Datatypes.None =>
      match findOneSubset (listset_to_listfset clauselist) (to_fset c) with
      | Init.Datatypes.None => True
      | Init.Datatypes.Some x2 => False
      end
  | Init.Datatypes.Some x2 =>
      match x2 with
      | (x3, x4) =>
          ((Init.Datatypes.Some (to_fset x3, x4)) =
...
Axiom H2 :
  ...
  match x1 with
  | RProp x2 =>
      ((x2 = top) -> (result1 = ATop)) /\ (~ (x2 = top) -> (result1 = ABot))
  | RVar x2 => (result1 = (AVar x2))
  end.

Axiom H3 :
  let result1 := ABot in
  let result2 := convertRStoAF x1 in
  ((eq_pos1 result2 result1) = Init.Datatypes.true) <-> (result2 = result1).

Axiom H4 : ~ ((eq_pos1 (convertRStoAF x1) ABot) = Init.Datatypes.true).
...
Axiom H7 : ((to_fset o) = (set.Fset.add (convertRStoAF x1) (to_fset c))).

Axiom H8 :
  let result1 := convertRStoAF x1 in
  (set.Fset.mem result1 (to_fset c) ->
...
Axiom H10 :
  ((to_fset result) =
   (algorithmA_spec full_clauselist
    ...
    (listset_to_listfset o1) (to_fset o))).
