...
Parameter clauselist: Init.Datatypes.list (set* rightside)%type.

Parameter c: set.

Axiom Requires : set.Fset.mem ATop (to_fset c).

Axiom Requires1 : ~ set.Fset.mem ABot (to_fset c).

Axiom Requires2 : sublist (listset_to_listfset clauselist) full_clauselist.
...
Axiom Requires5 : is_satisfiable_clauselist processed_clauses.
...
Parameter w: (set* rightside)%type.

Parameter w1: Init.Datatypes.list (set* rightside)%type.
...
Axiom Ensures :
  ((Init.Datatypes.Some (to_fset x, x1)) =
...
Axiom Ensures2 :
  match x1 with
  | RProp x2 =>
      ((x2 = top) -> ((convertRStoAF x1) = ATop)) /\
      (~ (x2 = top) -> ((convertRStoAF x1) = ABot))
  | RVar x2 => ((convertRStoAF x1) = (AVar x2))
  end.
...
Axiom H1 : ~ ((eq_pos1 (convertRStoAF x1) ABot) = Init.Datatypes.true).
...
Axiom Ensures6 :
  ((to_fset o) = (set.Fset.add (convertRStoAF x1) (to_fset c))).

Axiom Ensures7 :
  (set.Fset.mem (convertRStoAF x1) (to_fset c) ->
   ((set.Fset.cardinal (to_fset o)) = (set.Fset.cardinal (to_fset c)))) /\
  (~ set.Fset.mem (convertRStoAF x1) (to_fset c) ->
   ((set.Fset.cardinal (to_fset o)) =
    ((set.Fset.cardinal (to_fset c)) + 1%Z)%Z)).

Parameter o1: Init.Datatypes.list (set* rightside)%type.

Axiom Ensures8 :
  ((listset_to_listfset o1) =
   (removeFromList (to_fset x, x1) (listset_to_listfset clauselist))).

Axiom Requires8 : set.Fset.mem ATop (to_fset o).

Axiom Requires9 : ~ set.Fset.mem ABot (to_fset o).

Axiom Requires10 : sublist (listset_to_listfset o1) full_clauselist.

Axiom Requires11 :
  list.Permut.permut
  (Init.Datatypes.app (listset_to_listfset o1)
   (Init.Datatypes.cons (to_fset x, x1) processed_clauses))
  full_clauselist.

Axiom Requires12 :
  set.Fset.subset (to_fset o)
  (set.Fset.add ATop (set_smb_clauselist full_clauselist)).

Axiom Requires13 :
  is_satisfiable_clauselist
  (Init.Datatypes.cons (to_fset x, x1) processed_clauses).

Axiom Requires14 :
  forall (f:i -> b), ((eval_clauselist full_clauselist f) = top) ->
  ((eval_setconjunction (to_fset o) f) = top).

Parameter f: i -> b.

Axiom H2 : ((eval_setconjunction (to_fset o) f) = top).

(* Why3 goal *)
Theorem algorithmA'vc :
  ((eval_clauselist (Init.Datatypes.cons (to_fset x, x1) processed_clauses)
    f)
   = top).
