...
Parameter x: Init.Datatypes.list atom.

Parameter w: Init.Datatypes.list atom.

Parameter w1: Init.Datatypes.list (Init.Datatypes.list atom).

Axiom H : (phi = (Init.Datatypes.cons x (Init.Datatypes.cons w w1))).

Parameter result: formula.

Axiom Ensures :
  forall (f:i -> b),
  ((eval_cnf (Init.Datatypes.cons w w1) f) = (eval result f)) /\
  is_impl_free result /\ is_nnfc result /\ is_cnf result.

(* Why3 goal *)
Theorem to_formula'vc : ~ list.List.is_nil x.
auto.
